...
****  within a object extension literal, foo.{bar.{...}} adds properties to the existing
      value of foo.bar rather than replacing the value of foo.bar as done by foo.{bar:expr}}
...
******this is a slightly different parttern form that used in experiments 1-3*******
...
*** This version uses multiple prototype extensons per class to demonstrate a possible pattern
*** for grouping related methods.
...
export const Collection = AbstractClass <| function(n) {
	   ...
	   prototype.{ //enumerating protocol
	   ...
	   prototype.{ //private protocol for subclasses ("protected" in Java/c++ speak)
...
export const Set = Collection <| function(initialSize=2) {   
	   ...
	   prototype.{ //"protected" internal methods that access bagContents as a hash table
...
export const  SequenceableCollection = Collection <| function(){
    ...
    const BasicStorageCollection = SequenceableCollection <| function(elements) {
	  ...
	  this.{
	   ...
	   prototype.{ //"protected" methods for storage access
